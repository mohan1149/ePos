                    ...
                    <ul class="nav nav-pills nav-pills-icons" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link" href="#dashboard-1" role="tab" data-toggle="tab">
                                <img src="{{$category->category_avatar}}" alt="" class="category_avatar" width="35px" height="35px">
                                {{ $category->category }}
                            </a>
                        </li>
                        @endforeach
                ...
                <div class="row">
                                                ...
                                                <div class="col-md-2 col-lg-2" style="padding:5px">
                                                    <div class="card" style="margin: 0px !important">
                                                        <div class="card-header" style="padding: 0px">
                                                            <strong class="badge badge-primary">{{ number_format($product->price,$settings->decimal_points) }}</strong>
                                                        </div>
                                                        <button  data="{{ base64_encode($product) }}" class="btn btn-primary btn-link add-item-to-cart-btn" style="padding: 0px"><img src="{{$product->product_image}}" alt="" width="100%"></button>
                    ...
                    <div class="col-md-4 col-lg-4">
                        <div class="card p-3" style="height:50vh;overflow:scroll;">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <input type="text" class="form-control" id="order-for" required placeholder="{{ __('t.order_for') }}">
                      
                                </div>
                                <div class="col-md-6">
                                    <input type="text" class="form-control" id="discount" required placeholder="{{ __('t.discount') }}">
                                </div>
                            </div>
                            <div>
...
<script>
    ...
    $(document).ready(function(){
        $('.add-item-to-cart-btn').on('click',(element)=>{
            ...
            if(index === -1){
                if(product.stock_item === 1){
                    if(product.stock > 0){
                        ...
                        list+="</div>";
                        list+="<div>";
                        list+="<button onclick='decreaseItem("+product.id+")' class='btn btn-primary btn-fab btn-fab-mini btn-round'><i class='material-icons'>remove</i></button>";
                        list+="<span class='m-3' id='prduct_"+product.id+"_quantity'>"+product.quantity+"</span>";
                        list+="<button onclick='increaseItem("+product.id+")' class='btn btn-success btn-fab btn-fab-mini btn-round'><i class='material-icons'>add</i></button>";
                        list+="<button onclick='removeItem("+product.id+")' class='btn btn-danger btn-fab btn-fab-mini btn-round remove_item_from_cart'><i class='material-icons'>close</i></button>";
                        list+="</div>";
                        list+="</div></li>";
            ...
            const sum = cart_items.reduce((previousValue, currentValue) => previousValue + (currentValue.price * currentValue.quantity),0);
